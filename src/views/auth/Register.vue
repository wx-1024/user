    ...
    <div class="relative z-10 w-full max-w-lg">
      ...
      <div class="rounded-3xl border border-gray-200/80 bg-white/95 p-6 shadow-[0_24px_70px_-40px_rgba(15,23,42,0.45)] backdrop-blur-sm dark:border-white/10 dark:bg-[#111]/90 sm:p-8">
        <div class="mb-7 text-center">
          ...
          <h1 class="mt-3 text-3xl font-black text-gray-900 dark:text-white">{{ t('auth.register.title') }}</h1>
          <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">{{ t('auth.register.subtitle') }}</p>
        </div>
        ...
        <form class="space-y-5" @submit.prevent="handleRegister">
          ...
          <label class="flex items-start gap-3 rounded-xl border border-gray-200 bg-gray-50/70 px-4 py-3 text-sm text-gray-600 transition-colors dark:border-white/10 dark:bg-white/5 dark:text-gray-300">
            <input
              v-model="agreed"
              type="checkbox"
              class="mt-0.5 h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 dark:border-white/20 dark:bg-black/20"
            />
            <span class="leading-6">
              {{ t('auth.register.agreementPrefix') }}
              <router-link to="/privacy" target="_blank" rel="noopener noreferrer" class="font-semibold text-indigo-600 hover:text-indigo-500 dark:text-indigo-300 dark:hover:text-indigo-200">
                {{ t('footer.privacy') }}
              </router-link>
              {{ t('auth.register.agreementAnd') }}
              <router-link to="/terms" target="_blank" rel="noopener noreferrer" class="font-semibold text-indigo-600 hover:text-indigo-500 dark:text-indigo-300 dark:hover:text-indigo-200">
                {{ t('footer.terms') }}
              </router-link>
            </span>
          </label>
